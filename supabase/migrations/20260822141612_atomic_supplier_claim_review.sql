...
-- Production already has the profile-role hotfix. Fresh stacks may also
-- contain an equivalent boundary from their baseline history. Repeat the
-- prerequisite idempotently so the review RPC can never be exposed while a
-- user can promote themself, even if migration history has drifted.
