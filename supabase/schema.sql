...
-- Manual contact verification and delivery-event workflow (2026-07-14).
...
begin
  ...
  select exists (
    ...
    where recipient.venue_id = v_venue.id
      and recipient.status in ('pending', 'sent', 'delivered', 'failed', 'complained', 'replied', 'unsubscribed', 'suppressed')
...
-- Follow-up: draft recipients are not prior outreach.
