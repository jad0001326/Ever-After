...
export type SupplierCategorySlug = typeof supplierDirectoryCategories[number]["slug"];
...
export function supplierCategorySlugFromLabel(label: string) {
  ...
  return supplierDirectoryCategories.find(
    (category) => category.label.toLowerCase() === normalised,
  )?.slug ?? null;
}
