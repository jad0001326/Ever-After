...
| Track deposits, payments, instalments and deadlines | Budget item; Payments; Today | N6 and N8 | Total/over-allocation/date invariants, offline replay and overdue boundaries | Existing budget domain; implementation pending |
| Recommend the next logical action, with photography after venue selection | Today recommendation | N6–N8 | Venue-to-photography transition plus overdue payment/task precedence | Existing recommendation domain; implementation pending |
...
| Manage tasks | Plan > Tasks; Today | N8 | Stable-ID retry, collision, lost-response and idempotent CRUD tests | Dormant task API exists; implementation pending |
...
| Weekly return value | Today surfaces one reasoned action, closest payment/task deadline and progress | N6–N8 | Recommendation correctness plus useful-return product metric | Plan complete; implementation pending |
