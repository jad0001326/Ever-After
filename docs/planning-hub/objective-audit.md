...
| Reusable logic | Twenty-four declared budget, supplier, recommendation, dashboard and update-contract, profile, task, guest, seating and validation modules are guarded against React, Next.js, browser storage, Node runtime and Supabase adapter imports. The web layer maps platform-neutral recommendation targets to URLs; strict versioned JSON-safe facades and checked Draft 2020-12 schemas expose dashboard and profile reads plus conflict-safe profile, budget and atomic table-plan writes to future native presentation adapters. |
...
| Native iPhone and Android apps | Dashboard, profile, budget and table-plan API foundation prepared | The executable portable-domain boundary, platform-neutral recommendation DTOs and seven versioned language-neutral schemas now have dormant authenticated GET and PATCH adapters. All use the caller's RLS client; profile and budget writes require exact resource versions, while table-plan writes require the exact workspace version and use the existing atomic owner/partner transaction. Native applications, task mutations, physical-device QA and live Auth/Data API execution remain future gates. |
...
- 84 test files and 408 tests passing.
...
- The native-ready source boundary covers 24 declared domain modules and fails
