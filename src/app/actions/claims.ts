...
export async function submitSupplierClaim(_: ClaimState, formData: FormData): Promise<ClaimState> {
  ...
  const { data: supplier } = await supabase.from("supplier_listings").select("id, slug, is_claimed").eq("id", supplierId).eq("slug", slug).eq("category_slug", category.slug).eq("listing_status", "published").maybeSingle();
...
async function reviewSupplierClaim(formData: FormData, decision: "approved" | "rejected") {
  await requireAdmin();
  const claimId = requiredText(formData, "claimId");
  const supabase = await createClient();
  if (!supabase) redirect(`/admin/supplier-claims/${claimId}?message=Configure+Supabase+environment+variables+first`);
  const adminNotes = requiredText(formData, "adminNotes") || null;
  if (decision === "rejected" && !adminNotes) {
    redirect(`/admin/supplier-claims/${claimId}?message=Add+a+short+reason+before+rejecting+this+claim`);
  }

  const { data, error } = await supabase.rpc("review_supplier_claim", {
    p_claim_id: claimId,
    p_decision: decision,
  ...
  if (error || !reviewed) {
    redirect(`/admin/supplier-claims/${claimId}?message=${encodeURIComponent(error?.message ?? "This claim could not be reviewed")}`);
  }

  revalidatePath("/admin/supplier-claims");
  revalidatePath(`/admin/supplier-claims/${reviewed.reviewed_claim_id}`);
  revalidatePath("/vendor");
  const category = getPublicSupplierCategory(reviewed.category_slug);
  if (category) revalidatePath(publicSupplierProfilePath(category.slug, reviewed.supplier_slug));
  redirect(`/admin/supplier-claims/${reviewed.reviewed_claim_id}?message=Claim+${decision}`);
}

export async function approveSupplierClaim(formData: FormData) {
  return reviewSupplierClaim(formData, "approved");
}

export async function rejectSupplierClaim(formData: FormData) {
  return reviewSupplierClaim(formData, "rejected");
}
