...
export default async function EditSupplierPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ message?: string }> }) {
  ...
  const [{ data: supplier }, { data: photographer }, { data: outreachContact }] = await Promise.all([
    ...
    supabase.from("supplier_outreach_contacts").select("*").eq("supplier_id", id).maybeSingle()
  ]);
  if (!supplier) notFound();
  const publicCategory = getPublicSupplierCategory(supplier.category_slug);
  const publicProfile = supplier.listing_status === "published" && publicCategory
    ? publicSupplierProfilePath(publicCategory.slug, supplier.slug)
    : null;
  return <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8"><div className="flex flex-wrap items-center justify-between gap-3"><Link className="text-sm font-semibold text-[#35533e]" href="/admin/suppliers">Back to suppliers</Link>{publicProfile ? <Link className="text-sm font-semibold text-[#95502b]" href={publicProfile} target="_blank">View public profile</Link> : null}</div><p className="mt-6 text-sm font-semibold uppercase tracking-[0.18em] text-[#95502b]">Supplier directory</p><h1 className="mt-3 font-display text-5xl font-semibold">Edit {supplier.name}</h1>{message ? <p className="mt-5 rounded-2xl bg-[#eef4ea] px-4 py-3 text-sm text-[#285237]">{message}</p> : null}<div className="mt-8"><SupplierForm outreachContact={outreachContact} photographer={photographer} supplier={supplier} /></div></div>;
}
