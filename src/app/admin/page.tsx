...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ message?: string }> }) {
  const [{ message }, auth] = await Promise.all([searchParams, requireAdmin()]);
      ...
      <div className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 text-[var(--muted)]">
            {auth.demoMode ? "Demo mode: connect Supabase to enable protected writes." : "Protected admin area for venue operations."}
          </p>
        </div>
