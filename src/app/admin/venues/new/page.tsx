...
export default async function NewVenuePage({ searchParams }: { searchParams: Promise<{ message?: string }> }) {
  await requireAdmin();
  const { message } = await searchParams;
  const supabase = await createClient();
  const { data: amenities } = await supabase!.from("amenities").select("id, slug, name").order("name", { ascending: true });
