...
import { approveSupplierClaim, rejectSupplierClaim } from "@/app/actions/claims";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/field";
import { requireAdmin } from "@/lib/auth";
import { createAdminClient } from "@/lib/supabase/admin";

export const metadata: Metadata = { title: "Review supplier claim" };

export default async function SupplierClaimReviewPage({
  params,
  searchParams,
}: {
  ...
  const { data: claim } = await supabase!.from("supplier_claims").select("*").eq("id", id).single();
  if (!claim) notFound();
  const [{ data: supplier }, { data: audit }] = await Promise.all([
    supabase!.from("supplier_listings").select("*").eq("id", claim.supplier_id).single(),
    supabase!.from("supplier_claim_audit_log").select("*").eq("claim_id", claim.id).order("created_at", { ascending: false }),
  ]);
  const info = [
    ["Category", supplier?.category_slug ?? "Unknown"],
    ["Claimant", claim.claimant_name],
    ["Role", claim.claimant_role],
    ["Account email", claim.claimant_email],
    ["Business email", claim.business_email],
    ["Business phone", claim.business_phone],
    ["Evidence URL", claim.evidence_url || "Not provided"],
  ];

  return (
    <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
      <Link className="text-sm font-semibold text-[#5c6b52]" href="/admin/supplier-claims">Back to supplier claims</Link>
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_360px]">
        <section className="rounded-3xl border border-[var(--line)] bg-white p-6">
          ...
          <h1 className="mt-3 font-display text-5xl font-semibold">{supplier?.name ?? "Supplier claim"}</h1>
          <p className="mt-3 text-sm text-[var(--muted)]">Status: {claim.status}</p>
          {message ? <p className="mt-5 rounded-2xl bg-[#f4efe7] px-4 py-3 text-sm">{message}</p> : null}
          <dl className="mt-8 grid gap-5 sm:grid-cols-2">{info.map(([label, value]) => <div key={label}><dt className="text-xs font-semibold uppercase tracking-[0.18em] text-[#8a806f]">{label}</dt><dd className="mt-1 break-words text-sm text-[#4a443c]">{value}</dd></div>)}</dl>
