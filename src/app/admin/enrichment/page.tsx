...
import { applyAllSafeEnrichmentProposals } from "@/app/actions/enrichment";
import {
  EnrichmentMetric,
  ...
  return (
    <PageShell message={params.message}>
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        ...
        <EnrichmentMetric href={runHref(selectedRunId, { blocker: "currently_eligible" })} label="Eligible now" tone="success" value={summary.currentlyEligible} />
      ...
      <form action={applyAllSafeEnrichmentProposals} className="overflow-hidden rounded-3xl border border-[var(--line)] bg-white">
        <input name="runId" type="hidden" value={selectedRunId} />
        <div className="flex flex-col gap-4 border-b border-[var(--line)] px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted)]">Eligibility uses the current reviewed state. Audit projections remain visible separately; quality issues do not silently become send requirements.</p>
          </div>
          <div className="flex max-w-xl flex-col gap-3 sm:flex-row sm:items-center">
            <label className="flex items-start gap-2 text-xs leading-5 text-[var(--muted)]">
              ...
              {isCurrentRun ? `Confirm all ${summary.safeProposals} high-confidence, non-sensitive changes in this run. Contact, pricing and status changes remain individually safeguarded.` : "Historical runs are read-only; switch to the current completed run to apply safe changes."}
            </label>
            <Button disabled={!isCurrentRun || summary.safeProposals === 0} type="submit" variant="secondary">Apply all safe changes</Button>
          </div>
        </div>
  ...
  return (
    <article className="grid grid-cols-[1fr_auto] items-start gap-4 border-b border-[var(--line)] px-5 py-5 last:border-b-0 lg:grid-cols-[1.2fr_1fr_1fr_1fr_auto]">
      <div className="min-w-0">
        ...
        {record.safeProposals.length > 0 ? <p className="mt-3 text-xs text-[var(--muted)]">{record.safeProposals.length} high-confidence safe change{record.safeProposals.length === 1 ? "" : "s"}{allowBatchApproval ? " ready for the single apply action above." : " shown read-only."}</p> : null}
      </div>
      <div className="hidden lg:block"><EnrichmentStatus tone={record.currentEligible ? "success" : record.eligibleAfter ? "info" : "warning"}>{record.currentEligible ? "Eligible now" : record.eligibleAfter ? "Audit projection" : "Blocked"}</EnrichmentStatus><div className="mt-3"><EnrichmentTags values={record.currentBlockers} tone="danger" /></div></div>
  ...
  return records.map((record) => {
    ...
    return {
      ...
      businessName: enrichmentBusinessName(snapshot, profile) || "Unnamed business",
        currentEligible: hasBoolean(record.current_outreach_eligible) ? bool(record.current_outreach_eligible) : bool(record.before_outreach_eligible),
        eligibleBefore: bool(record.before_outreach_eligible), eligibleAfter: bool(record.after_outreach_eligible),
        currentBlockers: hasBoolean(record.current_outreach_eligible) ? strings(record.current_eligibility_blockers) : strings(record.eligibility_blockers), qualityBlockers: strings(record.quality_blockers), missingFields: strings(record.missing_fields),
      ...
      safeProposals: recordProposals.filter((proposal) => isSafeBatchEnrichmentProposal({ confidence: text(proposal.confidence), status: text(proposal.status), targetField: text(proposal.target_field) })).map((proposal) => ({ id: text(proposal.id), field: text(proposal.target_field) })),
       duplicate, suppressed: emailStatus === "suppressed" || (hasBoolean(record.current_outreach_eligible) ? strings(record.current_eligibility_blockers) : strings(record.eligibility_blockers)).includes("suppressed"), optedOut: emailStatus === "opted_out"
    };
  });
}
...
function filterRecords(records: ReviewRecord[], params: SearchParams) {
  ...
  return records.filter((record) => {
    ...
    if (params.blocker === "currently_eligible" && !record.currentEligible) return false;
    ...
    if (params.blocker && !["currently_eligible", "newly_eligible", "duplicate"].includes(params.blocker) && ![...record.currentBlockers, ...record.qualityBlockers, ...record.missingFields].includes(params.blocker)) return false;
    return true;
  });
}
...
function summarize(records: ReviewRecord[]) {
  return {
    total: records.length, currentlyEligible: records.filter((record) => record.currentEligible).length,
    newlyEligible: records.filter((record) => !record.eligibleBefore && record.eligibleAfter).length,
    missingEmail: records.filter((record) => record.currentBlockers.includes("missing_email") || record.missingFields.includes("email")).length,
    invalidEmail: records.filter((record) => record.emailStatus === "invalid" || record.currentBlockers.includes("invalid_email")).length,
    unverifiedEmail: records.filter((record) => record.emailStatus === "unverified" || record.currentBlockers.includes("unverified_contact")).length,
    ...
    safeProposals: records.reduce((total, record) => total + record.safeProposals.length, 0)
  };
}
