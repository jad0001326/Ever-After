...
function item(overrides: Partial<BudgetItem> = {}): BudgetItem { return { id: "item-1", categoryId: "venue", listingId: null, listingType: null, listingUrl: null, imageUrl: null, source: "manual", itemName: "Venue", supplierName: null, supplierType: null, description: null, estimatedCostPence: null, confirmedCostPence: null, importedPricePence: null, importedPriceToPence: null, importedPriceType: null, costPerPersonPence: null, guestCount: null, depositPaidPence: 0, totalPaidPence: 0, installments: [], costStatus: "estimated", paymentStatus: "not_started", bookingStatus: "researching", availabilityStatus: "not_checked", availabilityDate: null, dueDate: null, websiteUrl: null, notes: null, createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-01-01T00:00:00.000Z", sortOrder: 0, ...overrides }; }
describe("budget calculations", () => {
  ...
  it("calculates per-person pricing using the item guest count", () => expect(getItemPlanningCost(item({ costPerPersonPence: 8_500, guestCount: 80 }))).toEqual({ amountPence: 680_000, kind: "per_person" }));
  it("separates deposits and outstanding payments from remaining budget", () => { const plan = createEmptyBudgetPlan(); plan.totalBudgetPence = 2_000_000; plan.items = [item({ confirmedCostPence: 600_000, depositPaidPence: 100_000, totalPaidPence: 100_000 })]; expect(calculateBudget(plan)).toMatchObject({ remainingPence: 1_400_000, totalPaidPence: 100_000, outstandingPence: 500_000 }); });
  it("reserves percentage contingency and detects over-budget plans", () => { const plan = createEmptyBudgetPlan(); plan.totalBudgetPence = 1_000_000; plan.contingencyType = "percentage"; plan.contingencyValue = 5; plan.items = [item({ estimatedCostPence: 975_000 })]; expect(calculateBudget(plan)).toMatchObject({ contingencyPence: 50_000, spendableBudgetPence: 950_000, remainingPence: -25_000, health: "over" }); });
