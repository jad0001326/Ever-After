...
import checkedResourceSchema from "../../../docs/planning-hub/contracts/planning-table-plan-resource.v1.schema.json";
...
describe("Planning Table Plan API contracts", () => {
  ...
  it("accepts a strict read resource with the PATCH workspace version", () => {
    ...
    expect(planningTablePlanResourceSchema.parse(resource)).toEqual(resource);
    expect(planningTablePlanResourceSchema.safeParse({
      ...resource,
      memberRole: "partner",
    }).success).toBe(false);
  });
  ...
  it("keeps both checked language-neutral schemas current", () => {
    expect(checkedResourceSchema).toEqual(
      planningTablePlanResourceJsonSchema,
    );
    ...
    expect(checkedResourceSchema).toMatchObject({
      $id: "urn:everaft:planning-table-plan-resource:v1",
      additionalProperties: false,
    });
  });
});
