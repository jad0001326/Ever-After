...
describe("ExpenseDialog", () => {
  it("makes the full EverAft catalogue reachable instead of capping the picker at eight", () => {
    const listings: PlannerListing[] = Array.from({ length: 30 }, (_, index) => ({
      id: `venue-${index + 1}`,
      slug: `venue-${index + 1}`,
      name: `Venue ${index + 1}`,
      type: "Country Estate",
  ...
  it("shows a neutral labelled placeholder when a listing has no approved image", () => {
    const listing: PlannerListing = {
      id: "venue-no-image",
      slug: "venue-no-image",
      name: "Venue Without Photography",
      type: "Country Estate",
      categoryId: "venue",
      location: "Scotland",
      imageUrl: null,
      listingUrl: "/venues/venue-no-image",
      priceFromPence: null,
      priceToPence: null,
      pricingStatus: "unavailable",
    };

    render(<ExpenseDialog open plan={createEmptyBudgetPlan()} listings={[listing]} item={null} onClose={vi.fn()} onSave={vi.fn()} />);
    fireEvent.click(screen.getByRole("button", { name: "Select from EverAft" }));

    expect(screen.getByRole("img", { name: "Venue Without Photography has no approved image" })).toBeTruthy();
