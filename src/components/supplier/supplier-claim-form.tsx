...
export function SupplierClaimForm({
  supplierId,
  supplierSlug,
  categorySlug,
  categoryLabel,
  ...
  return <form action={formAction} className="rounded-3xl border border-[var(--line)] bg-white p-6">
    <input name="supplierId" type="hidden" value={supplierId} /><input name="slug" type="hidden" value={supplierSlug} /><input name="categorySlug" type="hidden" value={categorySlug} />
    ...
    <div className="mt-4 grid gap-4"><Field label="Evidence URL"><Input name="evidenceUrl" placeholder="Website team page, LinkedIn, or official contact page" type="url" /></Field><Field label="Message"><Textarea minLength={10} name="message" required placeholder={`Tell us how you are connected to this ${categoryLabel.toLowerCase()} business.`} /></Field></div>
