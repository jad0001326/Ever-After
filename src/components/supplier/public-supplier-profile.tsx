...
import { Building2, Calculator, Check, Clock3, ExternalLink, MapPin, WalletCards } from "lucide-react";
...
export function PublicSupplierProfile({
  ...
  const categoryPath = publicSupplierCategoryPath(supplier.categorySlug);
  const profilePath = publicSupplierProfilePath(supplier.categorySlug, supplier.slug);
  const claimPath = publicSupplierClaimPath(supplier.categorySlug, supplier.slug);
  const photographer = supplier.photographer;
  const breadcrumbSchema = buildBreadcrumbSchema([
    { name: "Home", path: "/" },
    { name: categoryPlural, path: categoryPath },
    { name: supplier.name, path: profilePath },
  ]);
  const structuredData = {
    ...
    priceRange: supplier.startingPricePence == null
      ? undefined
      : `From ${gbp.format(supplier.startingPricePence / 100)}`,
  ...
  return (
    <main>
      ...
      <section className="border-b border-[var(--line)] bg-[#f2ede4]">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-14">
          <div className="relative min-h-[24rem] overflow-hidden rounded-[1.75rem] bg-[#e9e0d2] lg:min-h-[38rem]">
            <SupplierVisual categoryLabel={categoryLabel} imageUrl={supplier.heroImageUrl} name={supplier.name} priority />
            ...
            {supplier.imagePermissionStatus === "representative" ? <p className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-[10px] font-semibold text-[#554a3d]">Representative image</p> : null}
          </div>
          <div className="self-center lg:pl-6">
            <Link className="text-sm font-semibold text-[#35533e]" href={categoryPath}>Back to {categoryPlural.toLowerCase()}</Link>
            <p className="mt-8 text-sm font-semibold uppercase tracking-[0.2em] text-[#95502b]">Wedding {categoryLabel.toLowerCase()}</p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {enquiryUrl ? <ButtonLink href={enquiryUrl} rel="noopener noreferrer" target="_blank">Visit website <ExternalLink size={16} /></ButtonLink> : null}
              <ButtonLink href={`/wedding-budget-planner?supplier=${encodeURIComponent(supplier.id)}`} variant="secondary"><Calculator size={16} /> Add to budget</ButtonLink>
            </div>
            <div className="mt-7 flex gap-3">
              {supplier.instagramUrl ? <SocialLink href={supplier.instagramUrl} label="Instagram" /> : null}
              {supplier.facebookUrl ? <SocialLink href={supplier.facebookUrl} label="Facebook" /> : null}
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)] lg:px-8 lg:py-20">
        <div>
          <section>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#95502b]">About</p>
            <h2 className="mt-3 font-display text-4xl font-semibold">Their approach</h2>
            <div className="mt-5 grid gap-4 text-base leading-8 text-[var(--muted)]">{supplier.description.split(/\n+/).filter(Boolean).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
          </section>
        ...
        <aside className="h-fit rounded-3xl border border-[var(--line)] bg-white p-6 lg:sticky lg:top-28">
          ...
          <dl className="mt-6 grid gap-5">
            <Detail label="Pricing" icon={<WalletCards size={17} />} value={pricingLabel(supplier)} />
            {photographer?.coverageHoursMin || photographer?.coverageHoursMax ? <Detail label="Coverage" icon={<Clock3 size={17} />} value={rangeLabel(photographer.coverageHoursMin, photographer.coverageHoursMax, "hours")} /> : null}
            {photographer?.turnaroundWeeksMin || photographer?.turnaroundWeeksMax ? <Detail label="Typical turnaround" icon={<Clock3 size={17} />} value={rangeLabel(photographer.turnaroundWeeksMin, photographer.turnaroundWeeksMax, "weeks")} /> : null}
          ...
          {!supplier.isClaimed ? <div className="mt-7 border-t border-[var(--line)] pt-5"><p className="text-sm font-semibold">Is this your business?</p><p className="mt-2 text-xs leading-5 text-[var(--muted)]">Verify your connection before access is granted.</p><ButtonLink className="mt-4 w-full" href={claimPath} variant="secondary">Claim this listing</ButtonLink></div> : null}
        </aside>
      </div>
    </main>
  );
...
function Detail({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="grid grid-cols-[2rem_1fr] gap-2"><span className="mt-0.5 text-[#9d7b45]">{icon}</span><div><dt className="text-xs font-semibold uppercase tracking-[0.14em] text-[#8a806f]">{label}</dt><dd className="mt-1 text-sm leading-6 text-[#403b34]">{value}</dd></div></div>;
}

function SocialLink({ href, label }: { href: string; label: string }) {
  return <a className="focus-ring inline-flex min-h-11 items-center gap-2 rounded-full border border-[var(--line)] bg-white px-4 text-sm font-semibold text-[var(--brand)] transition hover:bg-[#f7f1e8]" href={href} rel="noopener noreferrer" target="_blank">{label}<ExternalLink size={14} /></a>;
}
...
function pricingLabel(supplier: SupplierListing) {
  if (supplier.startingPricePence != null) return `Packages from ${gbp.format(supplier.startingPricePence / 100)}`;
  if (supplier.pricingUnit === "quote" && supplier.pricingSummary) return "Quote tailored to your plans";
