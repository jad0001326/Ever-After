...
export function SupplierVisual({
  ...
  if (imageUrl) {
    return <Image alt={`${name} wedding ${categoryLabel.toLowerCase()}`} className={cn("object-cover", className)} fill priority={priority} sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" src={imageUrl} />;
  }

  return (
    <div aria-label={`${name} EverAft profile cover. Approved portfolio imagery is being added.`} className={cn("absolute inset-0 grid place-items-center overflow-hidden bg-[#e9e0d2]", className)} role="img">
      ...
      <div className="relative text-center text-[#314537]">
        ...
        <span className="mt-1 block text-[10px] font-semibold uppercase tracking-[0.22em] text-[#746957]">EverAft {categoryLabel.toLowerCase()}</span>
      </div>
    </div>
  );
}
