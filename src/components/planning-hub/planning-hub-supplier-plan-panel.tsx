...
export function PlanningHubSupplierPlanPanel({
  ...
  selectedSupplier: PlanningHubSupplier | null;
  ...
  const hasBookedSupplier = supplierItems.some((item) => item.bookingStatus === "booked");
  const manualId = `manual-${category.slug}`;
  ...
  return (
    <aside aria-label="Connected wedding plan" className="self-start rounded-3xl border border-[#cfc3b3] bg-white lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto">
      ...
      <div className="border-b border-[#e4ddd2] p-5" data-testid="current-supplier-planning" id="current-supplier-planning">
        ...
        {selectedSupplier ? (
          <div className="mt-4 grid gap-3">
            ...
            <Label text={selectedSupplier.startingPricePence == null ? "Working estimate or quote" : "Planning cost"}>
              <MoneyInput ariaLabel={selectedSupplier.startingPricePence == null ? "Working estimate or quote" : "Planning cost"} onChange={onPlanningCostChange} value={planningCostPence} />
            </Label>
            {selectedSupplier.startingPricePence == null ? <p className="-mt-1 text-xs leading-5 text-[#625f57]">This profile is quote-only. Enter a working estimate now, then replace it when the quote arrives.</p> : null}
            <button className="focus-ring inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#173526] px-4 text-sm font-semibold text-white disabled:opacity-60" disabled={saveState === "saving"} onClick={onSupplierSave} type="button">
              {saveState === "saving" ? <Cloud className="animate-pulse" size={17} /> : selectedItem ? <Check size={17} /> : <Plus size={17} />}
              {selectedItem ? `Update ${category.label.toLowerCase()} plan` : `Add ${category.label.toLowerCase()} to plan`}
            </button>
          </div>
        ) : selectedItem ? (
      ...
      <details
        ...
        id={manualId}
        onToggle={(event) => setManualSupplierOpen(event.currentTarget.open)}
      ...
      >
        ...
        <form className="grid gap-3 px-5 pb-5" onSubmit={(event) => {
          ...
          if (!name) return;
          onManualSupplier(name, moneyFromForm(form.get("cost")), String(form.get("status")) as PlanningHubItemStatus);
          event.currentTarget.reset();
        }}>
          <Label text={`${category.label} name`}>
          ...
          <button className="focus-ring min-h-11 rounded-xl bg-[#173526] px-4 text-sm font-semibold text-white" type="submit">Add manual {category.label.toLowerCase()}</button>
        </form>
      </details>

      {supplierItems.length ? (
        <div className="border-b border-[#e4ddd2] p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#7a6d59]">{category.label} shortlist</p>
      ...
      <div className="p-5">
        ...
        <Link className="focus-ring mt-4 flex min-h-14 items-center justify-between gap-3 rounded-2xl bg-[#e8efe8] px-4 text-sm font-semibold text-[#173526]" href={withPlanningWorkspace("/planning-hub/organise", connectedWorkspaceId)} prefetch={false}>
          <span>{hasBookedSupplier ? "Next: organise guests and tables" : "Then: organise guests and tables"}<span className="mt-1 block text-xs font-normal text-[#5b665e]">{hasBookedSupplier ? `Your ${category.label.toLowerCase()} choice is connected.` : "Available whenever you are ready."}</span></span>
          <ArrowRight size={18} />
        </Link>
      </div>
    </aside>
...
function MoneyInput({ ariaLabel, onChange, value }: { ariaLabel: string; onChange: (pence: number) => void; value: number }) {
  return (
...
function MoneyField({ ariaLabel, name }: { ariaLabel: string; name: string }) {
  return (
    <span className="relative block">
      <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-[#625f57]">£</span>
      <input aria-label={ariaLabel} className="focus-ring min-h-11 w-full rounded-xl border border-[#cfc3b3] pl-7 pr-3 text-sm" inputMode="decimal" min={0} name={name} step="0.01" type="number" />
    </span>
  );
}
