...
export function PlanningHubPhotographyFilters({
  ...
  const hasFilters = Boolean(params.search || params.location || params.style || params.budget || params.sort);
      ...
      <form action="/planning-hub/photography" className="mt-5 grid gap-4">
        ...
        <Field label="Photography budget">
          <span className="relative block">
            ...
            <input className="focus-ring min-h-11 w-full rounded-xl border border-[#cfc3b3] pl-7 pr-3 text-sm" defaultValue={params.budget ?? ""} inputMode="decimal" min={0} name="budget" placeholder="2000" step="1" type="number" />
          </span>
        </Field>
        <Field label="Sort">
